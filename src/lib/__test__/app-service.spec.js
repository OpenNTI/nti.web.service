...
describe('lib/app-service', () => {
	...
	beforeEach(() => {
		...
		htmlAcceptsFilterMiddleware = jest.fn(
			() => 'htmlAcceptsFilterMiddleware'
		);
		apiProxyMiddleware = jest.fn(() => 'apiProxyMiddleware');
		cookieParserConstructor = jest.fn(() => 'cookie-parser-middleware');
		expressRequestLanguageMock = jest.fn(
			() => 'express-request-language-middleware'
		);
		...
		jest.doMock('../accepts-filters', () => ({
			htmlAcceptsFilter: htmlAcceptsFilterMiddleware,
		}));
		jest.doMock('../api', () => registerEndPoints);
		jest.doMock('../api-proxy', () => apiProxyMiddleware);
		jest.doMock('../compress', () => ({
			attachToExpress: compressionMock,
		}));
	...
	test('setupApplication(): gathers config, attaches middleware, and sets up all apps', async () => {
		...
		expect(service.setupClient).toHaveBeenCalledWith(
			config.apps[0],
			expect.any(Object)
		);
		expect(service.setupClient).toHaveBeenCalledWith(
	...
	test('setupClient(): expectations (production)', async () => {
		...
		const clientConfigWithAssets = {
			assets: mockReg.assets,
			devmode: false,
			...clientConfig,
		};
		...
		expect(staticMock).toHaveBeenCalledWith(
			mockReg.assets,
			expect.objectContaining({ maxAge: ONE_HOUR })
		);

		expect(clientApp.get).toHaveBeenCalledTimes(2);
		expect(clientApp.get).toHaveBeenCalledWith(
			expect.any(RegExp),
		...
		for (let n = 0; n < clientApp.use.mock.calls.length; n++) {
			...
			if (
				param === service.ANONYMOUS_ROUTES ||
				param === service.AUTHENTICATED_ROUTES
			) {
				fn.apply(null, args);
			}
		}

		expect(sessionMockInstance.anonymousMiddleware).toHaveBeenCalledTimes(
			1
		);
		expect(sessionMockInstance.anonymousMiddleware).toHaveBeenCalledWith(
		...
		expect(sessionMockInstance.middleware).toHaveBeenCalledWith(
			clientConfig.basepath,
			args[0],
			args[1],
			args[2]
		...
		expect(getPageRenderer).toHaveBeenCalledWith(
			clientConfigWithAssets,
			params.config,
			mockReg.render,
			undefined
		);
	...
	test('setupClient(): expectations (devmode)', async () => {
		...
		jest.doMock('test-app2', () => ({ register: jest.fn(() => mockReg) }), {
			virtual: true,
		});
