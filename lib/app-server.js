...
exports.setupApplication = (app, config) => {
	...
	for (let client of config.apps) {
		...
		registerEndPoints(
			...
			Object.assign({}, config, client),  //flattened config
			dsi.interface); //interface

		clientRoute.use(/^\/login/i, session.anonymousMiddleware.bind(session));

		//Session manager...
		clientRoute.use(/^(?!\/(api|login|resources)).*/i, session.middleware.bind(session));
